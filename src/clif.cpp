...
static bool check_filter(int flag, map_session_data *sd, struct block_list *tbl, map_session_data *tsd) {
...
static map_session_data *target_sd(struct block_list *bl) {
	nullpo_retr(NULL, bl);
	...
	switch(RBUFW(buf, 0)) {
		case 0x8d: // clif_GlobalMessage
			if (sd->block_chat > 0) {
				...
				map_session_data *tsd = target_sd(tbl);
...
#ifdef BGEXTENDED
void clif_sendfakenpc2(map_session_data *sd,map_session_data *fk) {
	unsigned char *buf;
	int fd = sd->fd;
	sd->state.using_fake_npc = 1;
	...
	case BL_MOB:
		{
			...
			if (md->option.hp_show == 1)
				clif_hpmeter_single(*sd, md->bl.id, md->status.hp, md->status.max_hp);
...
/// 0441 <skill id>.W
void clif_deleteskill(map_session_data *sd, int skill_id, bool flag)
{
...
void clif_wis_message(map_session_data* sd, const char* nick, const char* mes, int mes_len, int gmlvl){
	...
	WFIFOSET(fd,WFIFOW(fd,2));
	clif_soundeffect( ssd->bl, "kawai.wav", 0, SELF );
#endif
}
...
void clif_parse_BankOpen(int fd, map_session_data* sd) {
	...
	else if (pc_check_security(sd, SECU_OTHER))
		return;
	else {
...
#ifdef BGEXTENDED
	p.userNum = (bgd)?bgd->count:guild.connect_member;
#else
...
void clif_guild_memberlist( map_session_data& sd ){
	...
	if( battle_config.bg_eAmod_mode && sd.bg_id && map_getmapflag(sd.bl.m, MF_BATTLEGROUND) )
	{
		clif_bg_memberlist(sd); // TODO Checar esta madre xD
		return;
	}
#endif
	...
	switch( bl->type ){
		...
		case BL_MOB: {
			...
			if( md->guardian_data && md->guardian_data->guild_id ){
				...
				clif_send(&packet, sizeof(packet), src, target);
			}else if( battle_config.show_mob_info || md->option.hp_show > 1 && !map_getmapflag( bl->m, MF_HIDEMOBHPBAR )){				PACKET_ZC_ACK_REQNAMEALL packet = { 0 };
...
{
	pc_check_security_retv(sd, SECU_REMOVE_OPT);
...
void clif_parse_Auction_setitem(int fd, map_session_data *sd){
	...
	if (pc_check_security(sd, SECU_AUCTION)) {
...
void clif_bg_updatescore_team(map_session_data *sd)
{
	unsigned char buf[6];
	int i, m;
...
void clif_bg_leave(map_session_data *sd, const char *name, const char *mes)
{
	unsigned char buf[128];
	nullpo_retv(sd);
...
void clif_hat_effect_single(struct block_list* bl, uint16 effectId, bool enable) {
	...
	clif_send( p, p->packetLength, bl, SELF );
#endif
}

void clif_hat_effect_singleMvpBoard(map_session_data* sd, uint16 effectId, bool enable, enum send_target target = SELF) {
#if PACKETVER_MAIN_NUM >= 20150507 || PACKETVER_RE_NUM >= 20150429 || defined(PACKETVER_ZERO)
   nullpo_retv( sd );
   ...
   clif_send( p, p->packetLength, &sd->bl, target );
#endif
}
