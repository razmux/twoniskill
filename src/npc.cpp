...
void run_tomb(map_session_data* sd, struct npc_data* nd)
{
	...
	clif_scriptmes( *sd, nd->bl.id, msg_txt(sd, 1551));
	snprintf(buffer, sizeof(buffer), msg_txt(sd, 1552), nd->u.tomb.killer_name[0] ? nd->u.tomb.killer_name : "[Server]", FormatNumberForOutput(nd->u.tomb.damage1).c_str());
	clif_scriptmes( *sd, nd->bl.id, buffer );

	if (nd->u.tomb.damage2 != 0)
	{
	 snprintf(buffer, sizeof(buffer), msg_txt(sd, 1553), nd->u.tomb.p2, std::to_string(nd->u.tomb.damage2).c_str());
	  clif_scriptmes( *sd, nd->bl.id, buffer);
	}

	if (nd->u.tomb.damage3 != 0)
	{
	  snprintf(buffer, sizeof(buffer), msg_txt(sd, 1554), nd->u.tomb.p3, std::to_string(nd->u.tomb.damage3).c_str());
	  clif_scriptmes( *sd, nd->bl.id, buffer);
	}
...
e_purchase_result npc_buylist( map_session_data* sd, std::vector<s_npc_buy_list>& item_list ){
	...
	pc_check_security_retr(sd, SECU_NPCTRADE,
	e_purchase_result::PURCHASE_FAIL_COUNT);
...
const char *npc_get_script_event_name(int npce_index)
{
	...
	case NPCE_SKILLUSE: //Aglao - OnPCUseSkillEvent
