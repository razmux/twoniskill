...
void pc_bonus(map_session_data *sd,int type,int val)
{
	...
	switch(type){
		...
		case SP_SP_DRAIN_VALUE: // bonus bSPDrainValue,n;
			...
			break;
		case SP_MAGIC_HP_DRAIN_VALUE: // bonus bMagicHPDrainValue,n; //JinYuichi Rz
			if(!sd->state.lr_flag) {
			...
			break;
		case SP_MAGIC_SP_DRAIN_VALUE: // bonus bMagicSPDrainValue,n; //JinYuichi Rz
			if(!sd->state.lr_flag) {
...
void pc_bonus2(map_session_data *sd,int type,int type2,int val)
{
	...
	case SP_SP_DRAIN_RATE: // bonus2 bSPDrainRate,x,n;
		...
		break;
	case SP_MAGIC_HP_DRAIN_RATE: // bonus2 bHPDrainRate,x,n;  //JinYuichi Rz
		if(!sd->state.lr_flag) {
		...
		break;
	case SP_MAGIC_SP_DRAIN_RATE: // bonus2 bSPDrainRate,x,n;  //JinYuichi Rz
		if(!sd->state.lr_flag) {
	...
	case SP_SP_DRAIN_VALUE_CLASS: // bonus2 bSPDrainValueClass,c,n;
		...
		break;
	case SP_MAGIC_HP_DRAIN_VALUE_RACE: // bonus2 bMagicHPDrainValueRace,r,n;  //JinYuichi Rz
		PC_BONUS_CHK_RACE(type2,SP_MAGIC_HP_DRAIN_VALUE_RACE);
		if(!sd->state.lr_flag) {
		...
		break;
	case SP_MAGIC_SP_DRAIN_VALUE_RACE: // bonus2 bMagicSPDrainValueRace,r,n;  //JinYuichi Rz
		PC_BONUS_CHK_RACE(type2,SP_MAGIC_SP_DRAIN_VALUE_RACE);
		if(!sd->state.lr_flag) {
		...
		break;
	case SP_MAGIC_HP_DRAIN_VALUE_CLASS: // bonus2 bMagicHPDrainValueClass,c,n;  //JinYuichi Rz
		PC_BONUS_CHK_CLASS(type2,SP_MAGIC_HP_DRAIN_VALUE_CLASS);
		if(!sd->state.lr_flag) {
		...
		break;
	case SP_MAGIC_SP_DRAIN_VALUE_CLASS: // bonus2 bMagicSPDrainValueClass,c,n;  //JinYuichi Rz
		PC_BONUS_CHK_CLASS(type2,SP_MAGIC_SP_DRAIN_VALUE_CLASS);
		if(!sd->state.lr_flag) {
 ...
 *   7 = stack limitation
 */
enum e_additem_result pc_additem(map_session_data *sd,struct item *item,int amount,e_log_pick_type log_type, bool autostoring) {
	struct item_data *id;
	int16 i;
	unsigned int w;
...
bool pc_steal_item(map_session_data *sd,struct block_list *bl, uint16 skill_lv)
{
	...
	flag = pc_additem(sd,&tmp_item,1,LOG_TYPE_PICKDROP_PLAYER,true);
...
bool pc_revive_item(map_session_data *sd) {
	...
	if (map_getmapflag(sd->bl.m, MF_PVP) || map_getmapflag(sd->bl.m, MF_GVG) || map_getmapflag(sd->bl.m, MF_BATTLEGROUND)){
	 clif_displaymessage(sd->fd,msg_txt(sd,1550));
	 return false;
	}

	if (DIFF_TICK(gettick(), pc_readglobalreg(sd, add_str("TOKEN_OF_SIEGFRIED"))) < battle_config.tos_delay_use_time*1000) {
	 char atcmd_output[128];
	 clif_specialeffect(&sd->bl, EF_TEMP_FAIL, AREA);
	 sprintf(atcmd_output, "No puedes usar Token Of Siegried tan pronto, tiempo faltante: %ld", (DIFF_TICK(pc_readglobalreg(sd, add_str("TOKEN_OF_SIEGFRIED")), gettick())/1000)+battle_config.tos_delay_use_time);
	 clif_displaymessage(sd->fd,atcmd_output ); // Su chat global no está vinculado a ningún canal.
	 return false;
	}
	...
	pc_setglobalreg(sd, add_str("TOKEN_OF_SIEGFRIED"), gettick());
...
//==================================================
//==================================================
// Actualizacion del estado del PC
// en caso de recarga de la base de datos
// @param sd Player
// ================== JinRz ========================
//==================================================
 **/
...
// la bonificacion del script para un jugador
// @param sd Player
// ================== JinRz ========================
//==================================================
 **/
...
//==================================================
//==================================================
// Obtener la ubicacion de la base de datos
// de bonos de trabajo
// @param sd Player
// ================== JinRz ========================
//==================================================
...
*/
bool pc_check_security_(map_session_data *sd, enum e_security_check flag, bool notice) {
	...
	if (notice) {
		...
		switch (flag) {
			case SECU_DROP:				sprintf(output,msg_txt(sd,500),"soltar objeto(s)"); break;
			case SECU_VENDING:			sprintf(output,msg_txt(sd,500),"comprar articulo de vending"); break;
			case SECU_VENDING_OPEN:		sprintf(output,msg_txt(sd,500),"abrir una nueva tienda"); break;
			case SECU_BUYINGSTORE:		sprintf(output,msg_txt(sd,500),"vender artículo a la tienda de compra"); break;
			case SECU_BUYINGSTORE_OPEN:	sprintf(output,msg_txt(sd,500),"abrir una tienda de compra"); break;
			case SECU_TRADE:			sprintf(output,msg_txt(sd,500),"solicitar transaccion"); break;
			case SECU_GUILD_STORAGE:	sprintf(output,msg_txt(sd,500),"añadir/poner objeto en/desde el almacen del gremio"); break;
			case SECU_BREAKGUILD:		sprintf(output,msg_txt(sd,500),"romper un gremio"); break;
			case SECU_RESET_ITEM:		sprintf(output,msg_txt(sd,500),"borrar inventario/carro/almacenamiento"); break;
			case SECU_NPCTRADE:			sprintf(output,msg_txt(sd,500),"hacer transaccion con NPC"); break;
			case SECU_REMOVE_OPT:		sprintf(output,msg_txt(sd,500),"eliminar opcion"); break;
			case SECU_COMPOUND:			sprintf(output,msg_txt(sd,500),"componer una carta"); break;
			case SECU_DELHOMUN:			sprintf(output,msg_txt(sd,500),"eliminar homunculo"); break;
			case SECU_MAIL:				sprintf(output,msg_txt(sd,500),"utilizar el correo"); break;
			case SECU_AUCTION:			sprintf(output,msg_txt(sd,500),"subasta abierta"); break;
			case SECU_RESET_SKILL_STAT:	sprintf(output,msg_txt(sd,500),"reset skill/stats"); break;
			case SECU_FEEDING:			sprintf(output,msg_txt(sd,500),"alimentar homunculo/mascota"); break;
			default:					sprintf(output,msg_txt(sd,500),"realizar esta accion"); break;
		}
