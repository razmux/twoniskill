...
struct Script_Config script_config = {
	...
	"OnMvpSpawn", //mvpspawn_event_name
	// NPC related
...
BUILDIN_FUNC(areaeffect) {
	TBL_PC* sd;
	map_session_data* fk = (map_session_data*)aCalloc(1, sizeof(map_session_data));
...
BUILDIN_FUNC(hateffectmvp){
   ...
   if( !sd->state.connect_new ){
       clif_hat_effect_singleMvpBoard( sd, effectID, enable, static_cast<send_target>(target));
   }
...
BUILDIN_FUNC(mobevent)
{
	...
	if (class_ >= 0 && !mobdb_checkid(class_))
	{
		ShowWarning("buildin_mobevent: Attempted to spawn non-existing monster class %d\n", class_);
		return 1;
	}
	...
	script_pushint(st, k); // Id of mosnter / 0 if fail
